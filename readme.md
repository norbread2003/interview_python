...
[taizilongxu](https://github.com/taizilongxu/interview_pythonP)

## 修订内容

1. readme.md 保留面试时高频问答，可用于快速检索
2. 对于相关Python基础知识的详细解析，迁移至python_basis.md
3. 对于其他相关，如操作系统、网络等知识的详细解析，迁移至common_basis.md
4. 迁移高频面试算法题至coding.md，同时引入《编程之法》部分题目的Python解析
...
深拷贝(deep copy)：完全拷贝了父对象及其子对象。

对于简单对象，如：字符串、元组、一维数组，浅拷贝和深拷贝没区别。
对于复杂对象，如：二维数组，如果改变原对象的的子数组，新的浅拷贝对象会一起发生改变（此时新数组内的子数组实际上是对旧对象的引用），而深拷贝对象则不会受到影响。
...
重点是函数运行后并不会被撤销,就像instance字典一样,当函数运行完后,instance并不被销毁,而是继续留在内存空间里.这个功能类似类里的类变量,只不过迁移到了函数上.
